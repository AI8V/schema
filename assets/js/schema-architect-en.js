(function() {
    ...
    function convertToISODuration(text) {
        ...
        let hours = 0;
        let minutes = 0;
        const hourRegex = /(\d+)\s*(hour|hours)/i;
        const minuteRegex = /(\d+)\s*(minute|minutes)/i;
    ...
    function analyzeContent(html) {
        ...
        if (pageTitle && !entities.some(e => e.schemaProp === 'name')) {
             ...
             entities.push({ name: 'Page Title', value: pageTitle, schemaProp: 'name' });
        }
        return entities;
    }
    ...
    function analyzePrimaryEntities(doc) {
        ...
        if (date) {
            ...
            const displayDate = !isNaN(dateObj) ? dateObj.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : date;
            // i18n-key: entityNamePublishDate
            entities.push({ name: 'Publish Date', value: displayDate, schemaProp: 'datePublished', rawValue: dateObj.toISOString(), type: 'Article' });
        }
        return entities;
    }
    ...
    function analyzeFaqEntities(doc) {
        ...
        if (items.length > 0) {
             ...
             if (questions.length > 0) {
                 ...
                 return [{ name: contextualName || 'FAQ', value: `${questions.length} questions & answers`, schemaProp: 'mainEntity', type: 'FAQ', rawValue: questions, contextualName }];
             }
        }
        return [];
    }
    ...
    function analyzeProductEntities(doc) {
        ...
        if (priceEl) {
            ...
            if(price) {
                ...
                if (inferredCurrency) {
                     ...
                     entities.push({ name: 'Currency (Inferred)', value: inferredCurrency, schemaProp: 'priceCurrency', type: 'Product' });
                }
            }
        }
        ...
        if (currencyInput) { 
            ...
            entities.push({ name: 'Currency (Custom)', value: currencyInput, schemaProp: 'priceCurrency', type: 'Product' });
        }
    ...
    function analyzeRecipeEntities(doc) {
        ...
        if (ingredients.length > 0) {
            ...
            entities.push({ name: contextualName || 'Recipe', value: `${ingredientList.length} ingredients`, schemaProp: 'recipeIngredient', type: 'Recipe', rawValue: ingredientList, contextualName });
    ...
    function analyzeHowToEntities(doc) {
        ...
        if (steps.length > 0) {
            ...
            if (stepData.length > 0) {
                ...
                return [{ name: contextualName || 'Instructions', value: `${stepData.length} steps`, schemaProp: 'step', type: 'HowTo', rawValue: stepData, contextualName }];
            }
        }
        return [];
    }
    ...
    function analyzeEventEntities(doc) {
        ...
        if (startDateEl) {
            ...
            entities.push({ name: 'Event Start Date', value: dateValue, schemaProp: 'startDate', type: 'Event' });
    ...
    function analyzeOrganizationEntities(doc) {
        ...
        if (logoEl || addressElFromCustom) {
             ...
             if (logoEl) entities.push({ name: 'Organization Logo', value: new URL(logoEl.src, doc.baseURI).href, schemaProp: 'logo', type: 'Organization' });
             ...
             if (addressEl) {
                ...
                entities.push({ name: 'Organization Address', value: addressText.split('\n')[0], schemaProp: 'address', type: 'Organization' });
             }
             ...
             if (telephoneEl) {
                 ...
                 entities.push({ name: 'Organization Phone', value: phone, schemaProp: 'telephone', type: 'Organization' });
             }
        }
        return entities;
    }
    ...
    function suggestSchema(entities) {
        ...
        const suggestions = [{ type: 'WebPage', confidence: 0.5, reason: "The default option for any web page." }];
        ...
        hierarchy.forEach(type => {
            ...
            if (foundTypes.includes(typeToFind)) {
                ...
                if (type === 'Product') { reason = "Found product price data."; confidence = 0.98; }
                // i18n-key: suggestionReasonReview
                if (type === 'Review') { reason = "Found review rating data."; confidence = 0.97; }
                // i18n-key: suggestionReasonRecipe
                if (type === 'Recipe') { reason = "Found a list of ingredients."; confidence = 0.95; }
                // i18n-key: suggestionReasonHowTo
                if (type === 'HowTo') { reason = "Found a how-to/instructional step structure."; confidence = 0.92; }
                // i18n-key: suggestionReasonEvent
                if (type === 'Event') { reason = "Found an event date or location."; confidence = 0.90; }
                // i18n-key: suggestionReasonArticle
                if (type === 'Article') { reason = "Found a publish date or an author."; confidence = 0.85; }
                // i18n-key: suggestionReasonOrganization
                if (type === 'Organization') { reason = "Found organization data (logo, address)."; confidence = 0.80; }
                // i18n-key: suggestionReasonFaq
                if (type === 'FAQPage') { reason = "Found a question and answer structure."; confidence = 0.90; }
                // i18n-key: suggestionReasonBreadcrumb
                if (type === 'BreadcrumbList') { reason = "Found a breadcrumb navigation trail."; confidence = 0.96; }
...
function getPublisherData(entities) {
    ...
    function renderAnalysis(entities, suggestions) {
        ...
        let html = `<h3 class="h5 mb-3">1. Discovered Entities:</h3>`;
        if (entities.length === 0) {
            // i18n-key: analysisMessageNoEntities
            html += `<p class="text-muted small">No clear entities were discovered.</p>`;
        } else {
            entities.forEach(entity => {
        ...
        html += `<h3 class="h5 mt-4 mb-3">2. Suggested Schema Types:</h3>`;
    ...
    function updateActionButtonsState(isEnabled, copyText = 'Copy') {
        ...
        copyBtn.innerHTML = (copyText === 'Copy') ? `<i class="bi bi-clipboard-check me-1"></i> Copy` : copyText;
    }
        ...
        switch (type) {
            ...
            case 'Product':
                ...
                previewHtml = `
                    <div class="card">
                        <div class="card-body">
                            ${schema.image ? `<img src="${schema.image}" alt="Image of ${schema.name || 'the product'}" width="100" height="100" class="img-fluid rounded float-start me-3" style="max-width: 100px; height: auto; object-fit: cover;">` : ''}
                            ...
                            <p class="card-text">
                                ...
                                ${schema.brand ? `<span class="text-muted d-block">By: ${schema.brand.name}</span>` : ''}
                            </p>
                        </div>
                    </div>`;
                break;
            case 'Review':
                ...
                if(rating){
                 ...
                 previewHtml = `
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title">${item ? `Review for: ${item}` : (schema.name || 'A Review')}</h4>
                            ${rating ? `
                            <p class="card-text mb-1" title="Rating: ${rating} out of 5">
                                ${starsHtml}
                                <span class="ms-2 fw-bold">${rating} / 5</span>
                            </p>` : ''}
                            ${schema.author ? `<span class="text-muted d-block small">By: ${schema.author.name}</span>` : ''}
                        </div>
                    </div>`;
                break;
            case 'Recipe':
                 ...
                 previewHtml = `
                    <div class="card">
                        <div class="card-body">
                            ...
                            <h5 class="h6 small fw-bold">Ingredient Highlights:</h5>
            ...
            case 'Event':
                ...
                if(schema.startDate) {
                    try {
                        eventDate = new Date(schema.startDate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' });
                ...
                previewHtml = `
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title"><i class="bi bi-calendar-event me-2"></i>${schema.name || ''}</h4>
                            <p class="card-text small">
                                ${eventDate ? `<span class="d-block"><strong>Date:</strong> ${eventDate}</span>` : ''}
                                ${schema.location ? `<span class="d-block"><strong>Location:</strong> ${schema.location.name}</span>` : ''}
                                ${schema.organizer ? `<span class="d-block text-muted">Organizer: ${schema.organizer.name}</span>` : ''}
                            </p>
                        </div>
                    </div>`;
                break;
    ...
    async function handleAnalysis() {
        ...
        analysisResults.innerHTML = `<div class="text-center p-5"><div class="spinner-border text-primary" role="status"><span class="visually-hidden">Analyzing...</span></div><p class="mt-2">Decoding content...</p></div>`;
        generatedCode.value = '';
    ...
    async function fetchContent(url) {
        ...
        throw new Error(`Failed to fetch content. Check your internet connection or proxy status.`);
    }
            ...
            if (promptToCopy) {
                navigator.clipboard.writeText(promptToCopy)
                    .then(() => {
                        ...
                        copyEnhancedPromptBtn.innerHTML = '<i class="bi bi-check-lg me-2"></i> Prompt Copied Successfully!';
                        ...
                        setTimeout(() => {
                            ...
                            copyEnhancedPromptBtn.innerHTML = '<i class="bi bi-robot me-2"></i> Copy Full Enhancement Prompt';
