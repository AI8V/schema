...
/**
@file projects-en.js
@description Module for managing project templates (custom selector configurations) using localStorage. (English Version)
*/
...
function saveProject(projectName) {
    ...
    if (Object.keys(newProjectData).length === 0) {
        ...
        showToast('There are no custom identifiers to save. Please fill in at least one field.', 'info');
        return;
    }
    ...
    showToast(`Project "${projectName.trim()}" saved successfully.`, 'success');
}
...
function loadProject(projectName) {
    ...
    // If the project name is empty (user selected the default option)
    if (!projectName) {
